    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            ...
            if (len(random_image_label) < self.captcha_length):
                random_image_label = random_image_label.ljust(self.captcha_length, '/')
...
# def convertTF(saved_model_name):
        keras_model = tf.keras.models.load_model(args.output_model_name + '.h5')
        converter = tf.lite.TFLiteConverter.from_keras_model(keras_model)
        tflite_model = converter.convert()
        lite_model_path = args.output_model_name + '.tflite'
